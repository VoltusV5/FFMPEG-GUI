...
Сборка создаёт standalone-приложение (папка с exe и библиотеками) в каталоге, указанном в `exec_directory` в spec (по умолчанию — корень проекта). Иконка берётся из `icon.ico`; настройки Windows (имя продукта, версия) — из секции `[windows]` в `pysidedeploy.spec`.
